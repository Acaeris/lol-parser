...
namespace LeagueOfData\Service\Sql\Match;
...
/**
 * Match List object SQL factory.
 *
 * @package LeagueOfData\Service\Sql
 * @author  Caitlyn Osborne <acaeris@gmail.com>
 * @link    http://lod.gg League of Data
 */
class MatchCollection implements StoreServiceInterface
{
    ...
    public function add(array $matches)
    {
        foreach ($matches as $match) {
            ...
            $this->logger->error('Incorrect object supplied to Match service', [$match]);
        }
    }
    ...
    public function store()
    {
        ...
        foreach ($this->matches as $match) {
            if ($this->dbConn->fetchAll($select, $match->getKeyData())) {
                $this->dbConn->update('matches', $this->convertMatchToArray($match));
                continue;
            }
