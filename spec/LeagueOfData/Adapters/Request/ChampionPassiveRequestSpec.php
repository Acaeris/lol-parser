...
class ChampionPassiveRequestSpec extends ObjectBehavior
{
    public function let()
    {
    ...
    {
        $this->shouldHaveType('LeagueOfData\Adapters\Request\ChampionPassiveRequest');
    ...
    {
        $this->type()->shouldReturn('champion_passives');
    }

    public function it_cannot_be_used_for_a_json_request()
    {
        $this->requestFormat(Request::TYPE_JSON);
        $this->shouldThrow(new \Exception('Cannot create API query for passive alone'))
            ->during('query');
    }
    ...
    public function it_returns_the_correct_query_for_an_sql_request()
    {
        ...
        $this->query()->shouldReturn('SELECT test_column FROM champion_passives WHERE region = :region');
    }
    ...
    {
        $this->shouldNotThrow(new \InvalidArgumentException('Invalid ID supplied for Champion Passive request'))
            ->during('validate', [['champion_id' => 1]]);
    }

    public function it_can_validate_an_incorrect_id_parameter()
    {
        $this->shouldThrow(new \InvalidArgumentException('Invalid ID supplied for Champion Passive request'))
